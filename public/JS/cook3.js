...
/*document.addEventListener('DOMContentLoaded', () => {
    ...
    function validateRecipe(recipe) {
        ...
        const isValid = recipe.title &&
            recipe.description &&
            recipe.ingredients &&
            recipe.category !== 'Select a Category' &&
            recipe.tags.length > 0;

        if (!isValid) {
            console.error('Validation failed:', recipe);
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function saveRecipe(event) {
        ...
        if (validateRecipe(recipe)) {
             recipes.push(recipe);
            // Call the function to save the recipe to the backend
    ...
    function sendRecipeToDatabase(recipe) {
